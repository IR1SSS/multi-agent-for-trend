...
class DouYinLogin(AbstractLogin):
    ...
    async def popup_login_dialog(self):
        ...
        login_wait_ms = 120 * 1000
        try:
            await self.context_page.wait_for_selector(qrcode_selector, state="visible", timeout=login_wait_ms)
            return
        except PlaywrightTimeoutError:
            pass

        try:
            # check dialog box is auto popup and wait for login_wait_ms
            await self.context_page.wait_for_selector(dialog_selector, state="visible", timeout=login_wait_ms)
        except Exception as e:
            ...
            await login_button_ele.click(timeout=login_wait_ms, force=True)
            await self.context_page.wait_for_selector(qrcode_selector, state="visible", timeout=login_wait_ms)
