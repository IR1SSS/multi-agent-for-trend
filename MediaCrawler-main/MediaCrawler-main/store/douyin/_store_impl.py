...
class DouyinDbStoreImplement(AbstractStore):
    ...
    async def store_comment(self, comment_item: Dict):
        ...
        if comment_item.get("aweme_id") is not None:
            comment_item["aweme_id"] = int(comment_item["aweme_id"])
        if comment_item.get("like_count") is not None:
            comment_item["like_count"] = str(comment_item["like_count"])
        async with get_session() as session:
