...
DEFAULT_RUNTIME_CRAWL_TARGETS = ["douyin", "bilibili", "xiaohongshu"]
LOW_PRIORITY_CRAWL_TARGETS = {"xiaohongshu"}
...
def split_execution_sources(platforms: str | Iterable[str] | None) -> dict[str, list[str]]:
    ...
    return {
        "crawl_targets": _prioritize_crawl_targets(crawl_targets),
...
def _prioritize_crawl_targets(platforms: Iterable[str]) -> list[str]:
    primary: list[str] = []
    deferred: list[str] = []
    for platform in _unique_strings(platforms):
        if platform in LOW_PRIORITY_CRAWL_TARGETS:
