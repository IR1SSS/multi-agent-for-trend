...
class IntegrationRuntimeService:
    ...
    def __init__(self) -> None:
        ...
        self._inject_retryable_failure_once_remaining = False
    ...
    async def run(
        ...
        *,
        ...
        force_due_query_units: bool = False,
        inject_retryable_failure_once: bool = False,
    ) -> dict[str, Any]:
        await init_db()
        self._inject_retryable_failure_once_remaining = bool(inject_retryable_failure_once)

        run_id = datetime.now(timezone.utc).strftime("int002_%Y%m%d_%H%M%S_%f")
        ...
        requested_options = {
            ...
            "force_due_query_units": force_due_query_units,
            "inject_retryable_failure_once": inject_retryable_failure_once,
        }
        policy_overrides = {
            key: value
        ...
        try:
            ...
            for index, task in enumerate(scheduled_tasks):
                task_result = await self._run_task_with_transient_retry(
    ...
    async def _mark_batch_item_retry_dispatched(self, run_id: str, task: dict[str, Any]) -> None:
        config = task.get("config", {}) or {}
        query_unit_key = config.get("query_unit_key")
        if not query_unit_key:
            return
    ...
    async def _record_task_retry_event(
        self,
        *,
        batch_run_id: int,
        run_id: str,
        ...
        async with async_session_factory() as session:
            ...
            await service.add_event(
                ...
                keyword=task.get("keyword"),
                task_id=task.get("id"),
                dedup_key=config.get("task_dedup_key"),
                payload={
                ...
                message=f"Retry attempt {next_attempt_number} scheduled after retryable failure",
            )
            await session.commit()
    ...
    async def _run_task_with_transient_retry(
        self,
        *,
        batch_run_id: int,
        run_id: str,
        ...
        runtime_policy = task.get("_runtime_policy", {}) or {}
        max_attempts = max(1, int(runtime_policy.get("max_transient_attempts", 1) or 1))
        retry_backoff_seconds = max(0, int(runtime_policy.get("retry_backoff_seconds", 0) or 0))
        ...
        for attempt_number in range(1, max_attempts + 1):
            task_result = await self._run_task_pipeline(
                task["id"],
                max_raw_items=int(runtime_policy.get("max_raw_items", 1) or 1),
            )
            if self._inject_retryable_failure_once_remaining:
                self._inject_retryable_failure_once_remaining = False
                task_result = self._build_injected_retryable_failure(task_result)
            attempt_error = str(task_result.get("error", ""))
            retryable_error = (
                not bool(task_result.get("success", False))
                and RuntimeRecoveryService.is_retryable_error(attempt_error)
            )
            attempt_result = {
            ...
            await self._record_task_result(batch_run_id, run_id, {**task_result, "attempt_number": attempt_number})
            ...
            await self._record_task_retry_event(
                batch_run_id=batch_run_id,
                run_id=run_id,
                task=task,
            ...
            await self._mark_batch_item_retry_dispatched(run_id, task)
        ...
        if final_result is None:
            final_result = {
                ...
                "keyword_id": task.get("keyword_id", 0),
                "keyword": task.get("keyword", ""),
                "platform": task.get("platform", ""),
                "success": False,
                "cleaned_count": 0,
    ...
    @staticmethod
    def _build_injected_retryable_failure(task_result: dict[str, Any]) -> dict[str, Any]:
        return {
            **task_result,
            "success": False,
            "cleaned_count": 0,
            "signal_count": 0,
            "signal_output_file": "",
            "error": "injected_retryable_failure: page.goto timeout (test-only)",
        }
    ...
    async def _record_task_result(self, batch_run_id: int, run_id: str, task_result: dict[str, Any]) -> None:
        async with async_session_factory() as session:
            ...
            await service.add_event(
                ...
                task_id=task_result.get("task_id"),
                payload={
                    "attempt_number": int(task_result.get("attempt_number", 1) or 1),
        ...
        *,
        ...
        async with async_session_factory() as session:
            ...
            task_config_overrides = {
                ...
                "max_notes_count": effective_policy["max_notes_count"],
    ...
    def _render_markdown(self, report: dict[str, Any]) -> str:
        ...
        for item in report["task_results"]:
            ...
            retry_triggered = item.get("retry_triggered", False)
            lines.append(
                f"- task `{item['task_id']}` `{item['platform']}` `{item['keyword']}` success=`{str(item['success']).lower()}` attempts=`{attempt_count}` retry_triggered=`{str(bool(retry_triggered)).lower()}` cleaned=`{cleaned_count}` signals=`{signal_count}` error=`{error}`"
            )
            for attempt in item.get("attempts", []):
                lines.append(
                    f"  - attempt `{attempt.get('attempt_number', 0)}` success=`{str(bool(attempt.get('success', False))).lower()}` retryable_error=`{str(bool(attempt.get('retryable_error', False))).lower()}` cleaned=`{attempt.get('cleaned_count', 0)}` signals=`{attempt.get('signal_count', 0)}` error=`{attempt.get('error', '')}`"
                )
