...
class DomainConfig(Base):
    """Domain configuration — the central metadata table driving all system behavior.

    Each domain (e.g. beauty, new_energy_vehicle) has its own configuration
    for the three core skills: expander, cleaning, insight.
    Crawler platform selection is handled by the user at pipeline start time.
    All tasks in Celery carry a domain_id tag for logical isolation.
    """

    __tablename__ = "domain_configs"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    domain: Mapped[str] = mapped_column(String(64), nullable=False, unique=True, index=True, comment="Unique domain identifier, e.g. 'beauty', 'new_energy_vehicle'")
    display_name: Mapped[str] = mapped_column(String(128), nullable=False, comment="Human-readable domain name, e.g. '美妆护肤'")
    domain_description: Mapped[str] = mapped_column(Text, nullable=False, default="", comment="Natural language description of the domain for LLM-based skill inference")
    status: Mapped[str] = mapped_column(String(16), nullable=False, default="active", comment="Status: active / archived")
    expander_skill: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True, comment="Expander skill config: strategy, negative_keywords, max_depth, levels")
    cleaning_skill: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True, comment="Cleaning skill config: strategy, required_entities, noise_filters, eval_metrics, judge_enabled")
    insight_skill: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True, comment="Insight skill config: strategy, scoring_weights, anomaly_threshold_sigma, decay_delta, report_template")
...
class TrendKeyword(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    domain_id: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, index=True, comment="FK to domain_configs.id — logical domain isolation")
