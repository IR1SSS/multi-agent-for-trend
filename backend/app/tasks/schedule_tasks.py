...
def scheduled_crawl(platform: str, domain_id: int = 1) -> dict:
    ...
    Args:
        ...
        domain_id: The domain configuration ID (default: 1 for beauty).

    Returns:
        Dict with scheduling results.
    """
    return asyncio.run(_scheduled_crawl_async(platform, domain_id))


async def _scheduled_crawl_async(platform: str, domain_id: int = 1) -> dict:
    ...
    try:
        async with async_session_factory() as session:
            ...
            trend_context = AgentContext(platform=platform, domain_id=domain_id)
