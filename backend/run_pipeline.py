...
# ── DB Loader ──────────────────────────────────────────────────────────
async def load_keywords_from_db(domain_id: int, limit: Optional[int] = None) -> list[dict]:
    """Load keywords from trend_keywords table filtered by domain_id.
    ...
    async with async_session_factory() as session:
        stmt = select(TrendKeyword).where(
            TrendKeyword.domain_id == domain_id,
            TrendKeyword.is_active == True,  # noqa: E712
        ).order_by(TrendKeyword.id)
    ...
    logger.info(f"[DB] Loaded {len(keywords)} keywords for domain_id={domain_id}")
    return keywords
...
async def run_pipeline_once(
    ...
    Args:
        ...
        domain: Domain identifier for configuration (default: "beauty")
        source: Keyword source: "domain" (from DB) or "csv" (from CSV file)
    ...
    # Resolve domain_id from domain name
    domain_id = 1  # Default to beauty
    domain_description = ""
    try:
        from app.infrastructure.database.connection import async_session_factory
        ...
        async with async_session_factory() as session:
            ...
            domain_read = await repo.get_by_domain(domain)
            if domain_read:
                domain_id = domain_read.id
                domain_description = domain_read.domain_description or ""
    except Exception:
        pass

    # Load keywords from the selected source
    if source == "domain" and domain_id:
        keywords = await load_keywords_from_db(domain_id, limit=limit)
        # Fallback to CSV if no keywords found in DB
        if not keywords:
            csv_path = settings.TREND_KEYWORD_CSV
            logger.info(f"[Pipeline] No keywords in DB for domain_id={domain_id}, falling back to CSV")
    ...
    logger.info(f"  领域: {domain} (domain_id={domain_id})")
...
async def run_scheduled_pipeline(
    ...
    while True:
        ...
        await run_pipeline_once(mode=mode, platforms=platforms, login_type=login_type, headless=headless, domain=domain, source=source)
...
示例:
  ...
  python run_pipeline.py --mode prod --platform xhs --schedule daily --login-type cookie --headless
    ...
    parser.add_argument(
        ...
        type=str,
        default="beauty",
        help="领域标识符，决定使用的技能配置 (默认: beauty)",
    )
    parser.add_argument(
        "--source",
        choices=["domain", "csv"],
        default="domain",
        help="关键词来源: domain=从数据库读取, csv=从CSV文件读取 (默认: domain)",
    )
